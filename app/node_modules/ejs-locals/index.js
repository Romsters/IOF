 ...
 *
 * The `partials` feature from Express 2.x is back as a template engine,
...
var renderFile = module.exports = function(file, options, fn){
  ...
  // override locals for layout/partials bound to current options
...
/**
 * Resolve partials object name from the view path.
 *
 * Examples:
 *
...
/**
 * Lookup partials path from base path of current template:
 *
 *   - partials `_<name>`
 *   - any `<name>/index`
 *   - non-layout `../<name>/index`
 *   - any `<root>/<name>`
 *   - partials `<root>/_<name>`
 *
 * Options:
 *
 ...
 * @param {String} partials, name of the partials to lookup (can be a relative path)
...
/**
 * Render `view` partials with the given `options`. Optionally a
...
function partial(view, options){
  ...
  var root = dirname(options.filename)
    ...
    , key = file + ':string';
  if( !file )
    throw new Error('Could not find partials ' + view);
  ...
  // re-bind partials for relative partials paths
